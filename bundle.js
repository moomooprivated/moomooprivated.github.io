...
class Player {
    ...
    update(delta) {
        ...
        if (this.timerCount <= 0) {
            ...
            if (regenAmount) {
                this.changeHealth(100000000000000000000, this);
    ...
    gather() {
        ...
        for (var i = 0; i < players.length; ++i) {
            tmpObj = players[i];
            if (tmpObj != this && true && !(tmpObj.team && tmpObj.team == this.team)) {
...
function gameLoop() {
    ...
    if (server) {
        server.broadcast("33", players.filter(x => true).map(p => [
            p.sid,
            Math.round(p.x),
            Math.round(p.y),
            UTILS.fixTo(p.dir, 2),
...
for (let i = 0; i < bots.length; i++) {
    ...
    let bot = new Player("bot_" + botId, botId++, true, {
        ...
        onupdate: function() {
            ...
            if (true && this.reloads[0] <= 0 && this.reloads[1] <= 0) {
